...
class AccountScreen extends StatefulWidget {
  ...
  final bool isOwner;
  const AccountScreen({super.key, this.isOwner = false});
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (!mounted) return;
              // Về trang đăng nhập, xóa toàn bộ stack
                ...
                child: Container(
                  ...
                  child: const Row(
                    ...
                    children: [
                      ...
                      SizedBox(width: 4),
                      Text('Sửa', style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w500)),
                    ],
                  ),
                ),
              ),
                          ...
                          onPressed: isSaving ? null : () async {
                            ...
                            try {
                              ...
                              final response = await http.put(
                                Uri.parse('${ApiConstants.baseUrl}/api/users/$_userId'),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoRow(Icons.email_outlined, 'Email', _get('Email')),
          const Divider(height: 1, color: AppColors.divider, indent: 52),
          _buildInfoRow(Icons.phone_outlined, 'Số điện thoại', _get('Phone')),
          const Divider(height: 1, color: AppColors.divider, indent: 52),
          _buildInfoRow(Icons.location_on_outlined, 'Địa chỉ', _get('Location')),
          const Divider(height: 1, color: AppColors.divider, indent: 52),
          _buildInfoRow(Icons.cake_outlined, 'Ngày sinh', _get('DateOfBirth')),
          const Divider(height: 1, color: AppColors.divider, indent: 52),
          _buildInfoRow(Icons.wc_outlined, 'Giới tính', _get('Gender')),
        ],
      ),
    );
  }
